...
function StatCard({ icon: Icon, value, label, sub }: StatCardProps): JSX.Element {
  return (
...
function FounderDashboard(): JSX.Element {
  const founderCampaigns = campaigns.slice(0, 2) as Campaign[];
    ...
    <div className="space-y-6 sm:space-y-8 animate-fade-in">
      ...
      <div>
        <div className="flex items-center justify-between mb-4 sm:mb-5">
          ...
          <Link to="/create" className="flex items-center gap-1.5 text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors">
            + New <ArrowRight className="w-3 h-3" />
          </Link>
        </div>

        <div className="space-y-4">
          {founderCampaigns.map((campaign: Campaign) => {
              ...
              <div key={campaign.id} className="glass-card rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-white font-black text-base sm:text-lg leading-tight line-clamp-2">{campaign.title}</h3>
                  </div>
                  <Link to={`/campaign/${campaign.id}`} className="shrink-0 flex items-center gap-1 text-xs text-slate-400 hover:text-emerald-400 transition-colors font-medium bg-slate-800/50 px-2.5 py-1.5 rounded-lg">
                ...
                <div className="bg-slate-800/30 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-white/5 space-y-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Milestone</span>
                    {isReleased && (
                  ...
                  <p className="text-slate-300 text-xs sm:text-sm leading-relaxed line-clamp-2">{campaign.milestone.description}</p>
                  ...
                  {isVoting && (
                    <div className="space-y-2 pt-1">
                      ...
                      <div className="flex justify-between text-xs">
                        <span className="text-emerald-400 font-semibold">{yesPercent}% Yes ({campaign.milestone.voteYes})</span>
                        <span className="text-red-400 font-semibold">{100 - yesPercent}% No ({campaign.milestone.voteNo})</span>
                      </div>
                    </div>
                  )}
...
function InvestorDashboard(): JSX.Element {
  const investedCampaigns = (currentUser.investments as Investment[])
    .map((inv) => ({ ...inv, campaign: (campaigns as Campaign[]).find((c) => c.id === inv.campaignId) }))
    .filter((i): i is { campaign: Campaign; amount: number; tokens: number; campaignId: number } => !!i.campaign);

  const [votes, setVotes] = useState<Record<number, "yes" | "no">>({});

  const totalInvested = (currentUser.investments as Investment[]).reduce((s, i) => s + i.amount, 0);
  const totalTokens   = (currentUser.investments as Investment[]).reduce((s, i) => s + i.tokens, 0);
              ...
              <div key={campaign.id} className="glass-card rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-white font-black text-base sm:text-lg line-clamp-1">{campaign.title}</h3>
                  </div>
                  <Link to={`/campaign/${campaign.id}`} className="shrink-0 flex items-center gap-1 text-xs text-slate-400 hover:text-emerald-400 transition-colors font-medium bg-slate-800/50 px-2.5 py-1.5 rounded-lg">
                    <Eye className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline">View</span>
                ...
                <div className="bg-slate-800/30 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-white/5 space-y-3">
                  <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Milestone Vote</span>
                  <p className="text-slate-300 text-xs sm:text-sm line-clamp-2">{campaign.milestone.description}</p>
                  ...
                  {isVoting && !hasVoted && (
                    <div className="flex gap-2 sm:gap-3 pt-1">
                      ...
                      >
                        <ThumbsUp className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Approve
                      </button>
                      <button
                      ...
                      >
                        <ThumbsDown className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Reject
                      </button>
                    </div>
                  )}
                  ...
                  {isVoting && hasVoted && (
                    <div className={`flex items-center gap-2 p-3 rounded-xl text-xs sm:text-sm font-bold border ${
                      ...
                      {hasVoted === "yes" ? <ThumbsUp className="w-4 h-4 shrink-0" /> : <ThumbsDown className="w-4 h-4 shrink-0" />}
...
export default function Dashboard(): JSX.Element {
  ...
  const tabs: { id: "investor" | "founder"; label: string; labelFull: string; icon: LucideIcon }[] = [
    ...
    <div className="min-h-screen pb-24 relative">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12 relative z-10">
        <div className="mb-8 sm:mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-black text-white tracking-tight mb-2">My Dashboard</h1>
          <p className="text-slate-400 text-sm sm:text-base">
            {wallet ? (
              <span className="flex items-center gap-2 flex-wrap">
                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse shrink-0" />
                Logged in as{" "}
                <span className="text-emerald-400 font-mono font-bold break-all">{shortAddress}</span>
              </span>
            ) : (
        ...
        {!wallet ? (
          <div className="glass-card rounded-3xl p-8 sm:p-16 text-center">
            ...
            <h2 className="text-xl sm:text-2xl font-black text-white mb-3">Wallet Not Connected</h2>
