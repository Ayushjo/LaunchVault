...
interface MilestoneInput {
  description: string;
  bps: number; // 0-10000, must total 10000
}
      ...
      {STEPS.map((label, i) => (
        <div key={label} className="flex items-center">
          <div className="flex flex-col items-center gap-1.5">
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-body font-medium transition-all duration-300 ${
                i < current
                  ? "bg-white text-black"
                  : i === current
                  ? "liquid-glass-strong text-white ring-1 ring-white/30"
                  : "liquid-glass text-white/30"
              }`}
            >
              {i < current ? <CheckCircle2 className="w-3.5 h-3.5" /> : i + 1}
            </div>
            <span
              className={`text-[10px] font-body uppercase tracking-widest whitespace-nowrap ${
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`h-px w-16 md:w-24 mx-2 mb-5 transition-all duration-500 ${
...
function FieldLabel({ label, hint }: { label: string; hint?: string }) {
  return (
    <div className="flex items-center gap-1.5 mb-2">
      <span className="text-white/80 font-body font-medium text-sm">{label}</span>
      {hint && (
        <div className="group relative">
          <Info className="w-3.5 h-3.5 text-white/30 cursor-help" />
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 liquid-glass rounded-lg px-3 py-2 text-xs font-body text-white/70 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
            {hint}
          </div>
        </div>
      )}
  ...
  return (
    <div>
      <input
        ...
        className={`w-full liquid-glass rounded-xl px-4 py-3 text-sm font-body text-white placeholder-white/30 outline-none focus:ring-1 bg-transparent transition-all ${
          error ? "ring-1 ring-red-500/50" : "focus:ring-white/20"
        }`}
      />
      {error && (
...
function TextareaField({
  ...
  return (
    <textarea
      ...
      placeholder={placeholder}
      rows={rows}
      className="w-full liquid-glass rounded-xl px-4 py-3 text-sm font-body text-white placeholder-white/30 outline-none focus:ring-1 focus:ring-white/20 bg-transparent resize-none"
    />
  );
}
    ...
    <div className="min-h-screen bg-black pt-28 pb-20 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <button
          ...
          className="flex items-center gap-2 text-white/40 hover:text-white/70 font-body text-sm mb-8 transition-colors"
        >
        ...
        <p className="text-white/50 font-body font-light text-sm mb-10">
        ...
        {step === 0 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <TextareaField value={form.description} onChange={(v) => set("description", v)} placeholder="Describe your project, the problem it solves, and why you're the team to build it." rows={5} />
            </div>
            <div>
                ...
                {CATEGORIES.map((cat) => (
                  <button key={cat} onClick={() => set("category", cat)}
                    className={`px-4 py-2 rounded-full text-sm font-body font-medium transition-all ${
                      form.category === cat ? "bg-white text-black" : "liquid-glass text-white/60 hover:text-white"
                    }`}
                  >
                    {cat}
                  </button>
            ...
            {form.goal && !isNaN(parseFloat(form.goal)) && (
              <div className="liquid-glass rounded-xl px-4 py-4 flex items-start gap-3 border border-amber-500/20">
                ...
                <div>
                  ...
                  <p className="text-white/50 font-body font-light text-xs mt-1 leading-relaxed">
                    You must stake <span className="text-white font-medium">{requiredStake(form.goal)} ETH</span> (10% of goal) as collateral. This is slashed if investors reject a milestone — returned on successful completion.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-white/50 font-body font-light text-sm">
                Each milestone releases a portion of the raised funds when approved.
              </p>
              <button onClick={redistributeBps} className="text-white/40 hover:text-white/70 font-body text-xs transition-colors">
                Redistribute evenly
              </button>
            </div>

            {form.milestones.map((m, i) => (
              <div key={i} className="liquid-glass rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-white/50 font-body text-xs uppercase tracking-widest">Milestone {i + 1}</span>
                  {form.milestones.length > 1 && (
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div className="liquid-glass rounded-lg px-3 py-1.5 min-w-[56px] text-center">
            ...
            <button
              ...
              className="w-full liquid-glass rounded-xl py-3 flex items-center justify-center gap-2 text-white/40 hover:text-white/70 font-body text-sm transition-colors"
            >
            ...
            <div className={`flex items-center justify-between liquid-glass rounded-xl px-4 py-3 border ${bpsOk ? "border-emerald-500/30" : "border-amber-500/30"}`}>
              <span className="text-white/60 font-body text-sm">Total allocation</span>
        ...
        {step === 3 && (
          <div className="space-y-3">
            {[
              ...
              <div key={label} className="flex justify-between items-start liquid-glass rounded-xl px-4 py-3">
                <span className="text-white/40 font-body font-light text-sm">{label}</span>
                <span className="text-white font-body font-medium text-sm text-right max-w-xs">{value}</span>
              </div>
            ))}
            <div className="liquid-glass rounded-xl px-4 py-3 space-y-2">
              <span className="text-white/40 font-body font-light text-sm block mb-2">Milestones</span>
              {form.milestones.map((m, i) => (
                <div key={i} className="flex justify-between items-start gap-4">
                  <p className="text-white font-body text-sm flex-1 leading-relaxed">{m.description}</p>
                  <span className="text-white/50 font-body text-sm shrink-0">{(m.bps / 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
            <div className="liquid-glass rounded-xl px-4 py-4 border border-amber-500/20">
              <p className="text-amber-300 font-body font-medium text-sm mb-1">Before you launch</p>
              <p className="text-white/50 font-body font-light text-xs leading-relaxed">
                This will deduct <strong className="text-white">{requiredStake(form.goal)} ETH</strong> from your wallet as founder collateral. This is irreversible once the transaction confirms.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 mt-10">
          {step < STEPS.length - 1 ? (
            <button
              ...
              disabled={!canAdvance()}
              className="flex items-center gap-2 liquid-glass-strong rounded-full px-6 py-3 text-white font-body font-medium text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-white text-black rounded-full px-6 py-3 font-body font-medium text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:bg-white/90 transition-colors"
            >
              {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Launching…</> : "Launch Campaign"}
            </button>
          )}
          {!wallet && step === STEPS.length - 1 && (
            <p className="text-white/40 font-body text-xs">Connect your wallet to launch</p>
          )}
        </div>
      </div>
    </div>
