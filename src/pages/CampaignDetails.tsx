  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-between text-xs text-slate-400">
        <span className="text-emerald-400 font-semibold">{yesPercent}% Yes ({yes} votes)</span>
        <span className="text-red-400 font-semibold">{noPercent}% No ({no} votes)</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-3xl p-8 w-full max-w-md shadow-2xl">
        ...
        <div className="mb-6">
          <h2 className="text-xl font-black text-white mb-1">Invest in Campaign</h2>
        ...
        <div className="space-y-5">
          ...
          {amount && (
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-4 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-emerald-400 font-bold font-mono">{estimatedTokens} {campaign.tokens}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Ownership share</span>
                <span className="text-white font-bold">
                  {amount ? ((parseFloat(amount) / campaign.goal) * 100).toFixed(2) : 0}%
                </span>
              </div>
              <div className="flex justify-between text-sm">
          ...
          <div className="flex items-start gap-2 bg-amber-500/5 border border-amber-500/20 rounded-xl p-3">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Funds are locked in a smart contract and only released when you vote to approve milestones.
            </p>
          </div>

          <button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl transition-all duration-200 active:scale-95 shadow-lg shadow-emerald-900/40">
            Confirm Investment
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center px-4">
        <div>
          ...
          <h2 className="text-2xl font-black text-white mb-2">Campaign Not Found</h2>
          <p className="text-slate-400 mb-6">This campaign doesn't exist or has been removed.</p>
          <Link to="/" className="text-emerald-400 hover:text-emerald-300 font-bold flex items-center gap-2 justify-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="glass-card rounded-3xl p-8">
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/5">
                <div className="text-center">
                  ...
                  <div className="text-xl font-black text-white">{campaign.raised} ETH</div>
                ...
                <div className="text-center border-x border-white/5">
                  ...
                  <div className="text-xl font-black text-white">{campaign.investors}</div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-xl font-black text-white">{daysLeft > 0 ? `${daysLeft}d` : "Ended"}</div>
            ...
            <div className="glass-card rounded-3xl p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Fund release requires investor approval</p>
                </div>
                {milestone.released && (
              ...
              <div className="bg-slate-800/30 rounded-2xl p-5 mb-6">
                <p className="text-slate-300 font-medium leading-relaxed">{milestone.description}</p>
                <div className="flex items-center gap-4 mt-3">
                  <span className="text-xs text-slate-500">Release amount</span>
                  <span className="text-emerald-400 font-black">{milestone.amount} ETH</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {userVote && (
                  <div className={`flex items-center gap-2 p-4 rounded-xl border ${
                    ...
                    {userVote === "yes" ? <ThumbsUp className="w-4 h-4" /> : <ThumbsDown className="w-4 h-4" />}
                    <span className="font-bold text-sm">
                      You voted {userVote === "yes" ? "Yes" : "No"} — vote recorded
                    </span>
                  </div>
                )}
                ...
                {!milestone.votingActive && !milestone.released && (
                  <div className="flex items-center gap-2 text-slate-500 text-sm bg-slate-800/30 rounded-xl p-4">
                    ...
                    Voting hasn't started yet. The founder must request milestone release first.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="glass-card rounded-3xl p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{campaign.investors} backers on-chain</p>
                </div>
              </div>
                ...
                {mockInvestors.map((inv: Investor, i: number) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-slate-300 font-mono text-sm">{inv.address}</span>
                    ...
                    <div className="text-right">
                      <div className="text-white font-bold text-sm">{inv.amount} ETH</div>
                      <div className="text-emerald-400 font-mono text-xs">{inv.tokens} {campaign.tokens}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-5">
            <div className="glass-card rounded-3xl p-6 sticky top-24">
              <div className="mb-5">
                <div className="flex justify-between items-baseline mb-1">
                  <span className="text-3xl font-black text-white">{campaign.raised} ETH</span>
                  <span className="text-emerald-400 font-bold">{Math.round(progress)}%</span>
                </div>
                <p className="text-slate-500 text-sm mb-3">raised of {campaign.goal} ETH goal</p>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5 pt-4 border-t border-white/5">
                <div className="bg-slate-800/30 rounded-xl p-3 text-center">
                  <div className="text-lg font-black text-white">{campaign.investors}</div>
                  <div className="text-xs text-slate-500 mt-0.5">Backers</div>
                </div>
                <div className="bg-slate-800/30 rounded-xl p-3 text-center">
                  <div className="text-lg font-black text-white">{daysLeft > 0 ? `${daysLeft}d` : "—"}</div>
            ...
            <div className="glass-card rounded-3xl p-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wider mb-4">Token Info</h3>
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">Symbol</span>
                  <span className="text-emerald-400 font-bold font-mono">{campaign.tokens}</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            {/* Founder Info */}
            <div className="glass-card rounded-3xl p-6">
              <h3 className="text-sm font-black text-white uppercase tracking-wider mb-4">Founder</h3>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="text-slate-300 font-mono text-sm">{campaign.founder}</div>
